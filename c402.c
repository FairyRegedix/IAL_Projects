...
**                                         Implementoval : Patrik Demský, 2020
**
...
if((*RootPtr) == NULL) {
	...
	pozicia = NULL;
		...
		do{
			pozicia = pomocny;
			if(pomocny->Cont > Content) {
				pomocny = pomocny->LPtr;
			}
			else if(pomocny->Cont < Content) {
				pomocny = pomocny->RPtr;
			}
			else {
				nasiel = TRUE;
			}
		} while((!nasiel) && (pomocny));
	}
	if(nasiel != TRUE) {
...
**/
	...
	if (RootPtr == NULL){
		...
		Leftmost_Inorder(RootPtr, &stP);
		while (!SEmptyP(&stP)){
			RootPtr = STopPopP(&stP);
			BTWorkOut(RootPtr);
			Leftmost_Inorder(RootPtr->RPtr, &stP);
			}
		}
}
...
**/
	...
	while (SEmptyP(&stack_p) == FALSE) {
		tmp = STopPopP(&stack_p);
		if (STopPopB(&stack_b) == TRUE) {
			SPushP(&stack_p, tmp);
			SPushB(&stack_b, FALSE);
			Leftmost_Postorder(tmp->RPtr, &stack_p, &stack_b);
		} else {
			BTWorkOut(tmp);
		}
	}
